...
public class Wolf extends Enemy
{
    ...
    public Wolf(RefLinks refLink, float x, float y)
    {
        ...
        if(refLink.GetGame().level == 1)
        {
            ...
            SetSpeed(1.5f);             // Viteza inamicului de tip Wolf
            radiusOfSight = 100;        // Specificam raza vizuala a inamicului de tip Wolf
        ...
        if(refLink.GetGame().level == 2)
        {
            ...
            SetSpeed(1.6f);             // Viteza inamicului de tip Wolf
            radiusOfSight = 130;        // Specificam raza vizuala a inamicului de tip Wolf
            attackingDistance = 36;     // Specificam attackingDistance -> distanta de la care inamicul isi va incepe animatia de atac si va rani player-ul
            stunDuration = 60 / 2;      // Cat timp inamicul va fi ametit dupa o lovitura din partea eroului (aici 0.5 secunde)
        }
    ...
    public int setAttackAnimationID()
    {
        if(facingDirection == "Up")
        {
            if(animation.canAttack)
                return AnimationList.wolfAttackUp.ordinal();
            else
                return AnimationList.wolfIdleUp.ordinal();
        }
        if(facingDirection == "Down")
        {
            if(animation.canAttack)
                return AnimationList.wolfAttackDown.ordinal();
            else
                return AnimationList.wolfIdleDown.ordinal();
        }
        if(facingDirection == "Left")
        {
            if(animation.canAttack)
                return AnimationList.wolfAttackLeft.ordinal();
            else
                return AnimationList.wolfIdleLeft.ordinal();
        }
        if(facingDirection == "Right")
        {
            if(animation.canAttack)
                return AnimationList.wolfAttackRight.ordinal();
            else
                return AnimationList.wolfIdleRight.ordinal();
        }
    ...
    public void Update() throws FileNotFoundException
    {
        ...
        if(!WillCollideWithTiles() && !WillCollideWithItems())
        {
            ...
            {
                animation.setAnimID(AnimationList.wolfDeath.ordinal());
            }
    ...
    public void Draw(Graphics g)
    {
        ...
        if(refLink.GetGame().level == 2)
        {
            switch(GetLife())
            {
                ...
                case 3: { g.setColor(new Color(0, 180, 50)); break; }
                case 2: { g.setColor(new Color(200, 150, 0)); break; }
                case 1: { g.setColor(new Color(150, 20, 20)); break; }
                default: g.setColor(Color.white);
            }
    ...
    public int enemyMovement() //returneaza distanta dintre erou si inamic
    {
        ...
        if(dist < radiusOfSight)
        {
            ...
            if(xctrEnemy < xctrHero && yctrEnemy >= yctrHero && alreadyMoved == false)
            {
                ...
                {
                    animation.setAnimID(AnimationList.wolfWalkRight.ordinal());
                    facingDirection = "Right";
                }
                else
                {
                    animation.setAnimID(AnimationList.wolfWalkUp.ordinal());
                    facingDirection = "Up";
                }
            ...
            if(xctrEnemy > xctrHero && yctrEnemy < yctrHero && alreadyMoved == false)
            {
                ...
                if(Math.abs(xctrEnemy - xctrHero) > Math.abs(yctrEnemy - yctrHero))
                {
                    animation.setAnimID(AnimationList.wolfWalkLeft.ordinal());
                    facingDirection = "Left";
                }
                else
                {
                    animation.setAnimID(AnimationList.wolfWalkDown.ordinal());
            ...
            {
                animation.setAnimID(AnimationList.wolfIdleDown.ordinal());
