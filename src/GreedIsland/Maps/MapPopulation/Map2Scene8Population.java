...
public class Map2Scene8Population extends MapPopulation
{
    private static Map2Scene8Population map2S8Pop = null;

    public Map2Scene8Population(RefLinks refLink)
    {
        items = new ArrayList<Item>();
        enemies = new ArrayList<Enemy>();

        // Aici vom adauga itemele concrete care se afla in Map1 Scene1 (cea cu casa) (la inceputul jocului)
        Item chest1 = new Chest(refLink, 13*32, 3*32, 32, 32, ChestActionType.giveTreasure.ordinal());
        items.add(chest1);

        // Aici vom adauga inamicii concreti care se afla in Map1 Scene1 (la inceputul jocului)
        Enemy enemy1 = new Wolf(refLink, 11*32, 11*32);
        Enemy enemy2 = new Orc(refLink,16*32, 2*32);
        enemies.add(enemy1);
        enemies.add(enemy2);
    }
    ...
    public static Map2Scene8Population getInstance(RefLinks refLink)
    {
        if(map2S8Pop == null)
        {
            synchronized (Map2Scene8Population.class)
            {
                if(map2S8Pop == null)
                    map2S8Pop = new Map2Scene8Population(refLink);
            }
        }
