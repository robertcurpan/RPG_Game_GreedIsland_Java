...
/*! \class public class Map1Scene4Population implements MapPopulation
...
public class Map1Scene4Population extends MapPopulation
{
    private static Map1Scene4Population map1S4Pop = null;

    public Map1Scene4Population(RefLinks refLink)
    {
        items = new ArrayList<Item>();
        enemies = new ArrayList<Enemy>();

        // Aici vom adauga itemele concrete care se afla in Map1 Scene3
        Item chest1 = new Chest(refLink, 6*32, 9*32, 32, 32, ChestActionType.giveLife.ordinal());
        Item chest2 = new Chest(refLink, 17*32, 1*32, 32, 32, ChestActionType.giveLife.ordinal());
        items.add(chest1);
        items.add(chest2);

        // Aici vom adauga inamicii concreti care se afla in Map1 Scene3
        Enemy enemy1 = new Skeleton(refLink, 11*32, 16*32);
        Enemy enemy2 = new Wolf(refLink, 11*32, 5*32);
        enemies.add(enemy1);
        enemies.add(enemy2);
    }
    ...
    public static Map1Scene4Population getInstance(RefLinks refLink)
    {
        if(map1S4Pop == null)
        {
            synchronized (Map1Scene4Population.class)
            {
                if(map1S4Pop == null)
                    map1S4Pop = new Map1Scene4Population(refLink);
            }
        }
