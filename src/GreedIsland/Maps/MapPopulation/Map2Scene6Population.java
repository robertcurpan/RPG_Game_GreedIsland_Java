...
public class Map2Scene6Population extends MapPopulation
{
    private static Map2Scene6Population map2S6Pop = null;

    public Map2Scene6Population(RefLinks refLink)
    {
        items = new ArrayList<Item>();
        enemies = new ArrayList<Enemy>();

        // Aici vom adauga itemele concrete care se afla in Map1 Scene1 (cea cu casa) (la inceputul jocului)
        Item chest1 = new Chest(refLink, 24*32, 5*32, 32, 32, ChestActionType.giveLife.ordinal());
        Item chest2 = new Chest(refLink, 2*32, 9*32, 32, 32, ChestActionType.giveLife.ordinal());
        items.add(chest1);
        items.add(chest2);

        // Aici vom adauga inamicii concreti care se afla in Map1 Scene1 (la inceputul jocului)
        Enemy enemy1 = new Wolf(refLink, 16*32, 4*32);
        Enemy enemy2 = new Orc(refLink, 10*32, 13*32);
        enemies.add(enemy1);
        enemies.add(enemy2);
    }
    ...
    public static Map2Scene6Population getInstance(RefLinks refLink)
    {
        if(map2S6Pop == null)
        {
            synchronized (Map2Scene6Population.class)
            {
                if(map2S6Pop == null)
                    map2S6Pop = new Map2Scene6Population(refLink);
            }
        }
