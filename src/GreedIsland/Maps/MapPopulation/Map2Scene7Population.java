...
public class Map2Scene7Population extends MapPopulation
{
    private static Map2Scene7Population map2S7Pop = null;

    public Map2Scene7Population(RefLinks refLink)
    {
        items = new ArrayList<Item>();
        enemies = new ArrayList<Enemy>();

        // Aici vom adauga itemele concrete care se afla in Map1 Scene1 (cea cu casa) (la inceputul jocului)
        Item chest1 = new Chest(refLink, 2*32, 5*32, 32, 32, ChestActionType.giveLife.ordinal());
        Item chest2 = new Chest(refLink, 2*32, 14*32, 32, 32, ChestActionType.giveKey.ordinal());
        items.add(chest1);
        items.add(chest2);

        // Aici vom adauga inamicii concreti care se afla in Map1 Scene1 (la inceputul jocului)
        Enemy enemy1 = new Wolf(refLink, 10*32, 6*32);
        Enemy enemy2 = new Orc(refLink, 5*32, 6*32);
        Enemy enemy3 = new Orc(refLink, 10*32, 13*32);
        Enemy enemy4 = new Orc(refLink, 4*32, 13*32);
        enemies.add(enemy1);
        enemies.add(enemy2);
        enemies.add(enemy3);
        enemies.add(enemy4);
    ...
    public static Map2Scene7Population getInstance(RefLinks refLink)
    {
        if(map2S7Pop == null)
        {
            synchronized (Map2Scene7Population.class)
            {
                if(map2S7Pop == null)
                    map2S7Pop = new Map2Scene7Population(refLink);
            }
        }
